...
# Capacidade máxima de agendamentos por hora
LIMITE_AGENDAMENTOS_POR_HORA = 2
...
class Agendamento(models.Model):
    ...
    def clean(self):
        ...
        if self.data and self.data < timezone.now():
            ...
            inicio_hora = self.data.replace(minute=0, second=0, microsecond=0)
            fim_hora = inicio_hora + timedelta(hours=1)
 
            agendamentos_no_horario = Agendamento.objects.filter(
                data__gte=inicio_hora,
                data__lt=fim_hora,
            ).exclude(status="Cancelado")
 
            if self.pk:
                agendamentos_no_horario = agendamentos_no_horario.exclude(pk=self.pk)
 
            if agendamentos_no_horario.count() >= LIMITE_AGENDAMENTOS_POR_HORA:
                erros["data"] = (
                    f"Já existem {LIMITE_AGENDAMENTOS_POR_HORA} agendamentos nesse horário. "
